...
def reactor_efficiency(voltage, current, theoretical_max_power):
    """
        ...
        The percentage value is calculated as (generated power / theoretical max power) * 100 where generated power = voltage * current

    """

    generated_power = voltage * current
    efficiency_as_percentage = (generated_power / theoretical_max_power) * 100
    efficiency_band = None
    
    if efficiency_as_percentage >= 80:
        efficiency_band = "green"
    elif 60 <= efficiency_as_percentage < 80:
        efficiency_band = "orange"
    elif 30 <= efficiency_as_percentage < 60:
        efficiency_band = "red"
    else:
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if product_of_temp_and_neutrons < (0.9 * threshold):
        output_status = "LOW"
    elif 0.9 * threshold <= product_of_temp_and_neutrons <= 1.1 * threshold:
